...
int main(void)
{
	...
	//iprintf("el valor de x es: %d\r\n", x); /*debe llevar fin de carro y nueva línea para que vuelque el buffer*/
	/*
	snprintf(buffer, 100, "el valor de x es: %d\r\n", x);
	printf(buffer);*/
...
writes the formatted data to stdout, without a terminating NUL character. printf returns
when it reaches the end of the format string.
iprintf' is a restricted version of `printf': it has the same arguments and behavior,
save that it cannot perform any floating-point formatting: the `f', `g', `G', `e', and `F' type
...
field is interpreted according to a format specifier passed to scanf
